...
def note_list(request, calendar_pk):
    calendar = get_object_or_404(Schedule, pk=calendar_pk)
    notes = ClientEventNote.objects.filter(calendar=calendar)
    return render(request, 'schedule/note_list.html', {'notes': notes})